...
func TestAdopt_JSONDoesNotForceSourceConflict(t *testing.T) {
	sb := testutil.NewSandbox(t)
	defer sb.Cleanup()
	...
	if err := json.Unmarshal([]byte(result.Stdout), &payload); err != nil {
		t.Fatalf("invalid adopt JSON: %v\nstdout: %s", err, result.Stdout)
	}
	if len(payload.Adopted) != 0 || len(payload.Skipped) != 1 || payload.Skipped[0] != "firecrawl" {
		t.Fatalf("unexpected conflict result: adopted=%v skipped=%v", payload.Adopted, payload.Skipped)
	}
	...
	if string(content) != "# Source Version" {
		t.Fatalf("source overwritten by --json: %q", content)
	}
	if !sb.FileExists(filepath.Join(agentsPath, "firecrawl", "SKILL.md")) {
		t.Fatal("conflicting original was trashed by --json")
	}
}

func TestAdopt_ForceRefusesInstalledSourceOwnership(t *testing.T) {
	sb := testutil.NewSandbox(t)
	defer sb.Cleanup()

	sb.CreateSkill("firecrawl", map[string]string{"SKILL.md": "# Installed Version"})
	store := install.NewMetadataStore()
	store.Set("firecrawl", &install.MetadataEntry{Source: "github.com/example/firecrawl"})
	...
	result := sb.RunCLI("adopt", "-g", "--all", "--force")
	result.AssertSuccess(t)
	result.AssertAnyOutputContains(t, "managed by install metadata")

	content, err := os.ReadFile(filepath.Join(sb.SourcePath, "firecrawl", "SKILL.md"))
	if err != nil || string(content) != "# Installed Version" {
		t.Fatalf("installed source changed: content=%q err=%v", content, err)
	}
	if !sb.FileExists(filepath.Join(agentsPath, "firecrawl", "SKILL.md")) {
		t.Fatal("external original was trashed despite managed ownership")
	}
	loaded, err := install.ReadMetadata(sb.SourcePath)
	if err != nil || loaded.GetByPath("firecrawl") == nil {
		t.Fatalf("install metadata was removed: store=%+v err=%v", loaded, err)
	}
}
...
func TestAdoptProject_ForceRefusesConfigReplayOwnership(t *testing.T) {
	sb := testutil.NewSandbox(t)
	defer sb.Cleanup()
...
skills:
  - name: firecrawl
    source: github.com/example/firecrawl
`)
	agentsPath := filepath.Join(projectRoot, ".agents", "skills")
	adoptWriteSkill(t, agentsPath, "firecrawl")

	result := sb.RunCLIInDir(projectRoot, "adopt", "-p", "--all", "--force")
	result.AssertSuccess(t)
	result.AssertAnyOutputContains(t, "managed by project install replay")

	projectSource := filepath.Join(projectRoot, ".skillshare", "skills")
	if sb.FileExists(filepath.Join(projectSource, "firecrawl")) {
		t.Fatal("config-managed destination was created")
	}
	if !sb.FileExists(filepath.Join(agentsPath, "firecrawl", "SKILL.md")) {
		t.Fatal("external original was trashed despite project replay ownership")
	}
	if _, err := os.Stat(filepath.Join(projectSource, install.MetadataFileName)); !os.IsNotExist(err) {
		t.Fatalf("adopt created metadata while refusing project ownership: %v", err)
	}
}
