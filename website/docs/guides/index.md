---
sidebar_position: 1
---

# Guides

In-depth guides for specific use cases.

## What do you want to do?

| I want to... | Read |
|--------------|------|
| Write my first skill | [Creating Skills](./creating-skills.md) |
| Design reliable, deterministic skills | [Skill Design](./skill-design.md) |
| Organize a growing skill collection | [Organizing Skills](./organizing-skills.md) |
| Share skills with my team via a git repo | [Organization-Wide Skills](./organization-sharing.md) |
| Set up skills for a specific project | [Project Setup](./project-setup.md) |
| Sync skills across multiple machines | [Cross-Machine Sync](./cross-machine-sync.md) |
| Browse and discover community skills | [Hub Index](./hub-index.md) |
| Secure my skill supply chain | [Securing Your Skills](./security.md) |
| Move from another tool or approach | [Migration](./migration.md) |
| Test skillshare in an isolated Docker sandbox | [Docker Sandbox](./docker-sandbox.md) |

## By Topic

### Skills

| Guide | Description |
|-------|-------------|
| [Creating Skills](./creating-skills.md) | Create and publish your own skills |
| [Skill Design](./skill-design.md) | Complexity levels, determinism, CLI wrapper pattern |
| [Organizing Skills](./organizing-skills.md) | Organize skills into folders with auto-flattening |
| [Best Practices](./best-practices.md) | Naming, organization, and versioning |

### Sharing

| Guide | Description |
|-------|-------------|
| [Project Setup](./project-setup.md) | Set up project-scoped skills for a repo |
| [Organization-Wide Skills](./organization-sharing.md) | Share skills across your organization |
| [Cross-Machine Sync](./cross-machine-sync.md) | Sync skills across computers |
| [Hub Index](./hub-index.md) | Browse and manage skill hubs |

### Advanced

| Guide | Description |
|-------|-------------|
| [Migration](./migration.md) | Migrate from other tools or between modes |
| [Comparison](./comparison.md) | Compare skill management approaches |
| [Docker Sandbox](./docker-sandbox.md) | Isolated test sandbox and interactive playground |
| [Securing Your Skills](./security.md) | Security scanning, custom rules, and organizational policy |
