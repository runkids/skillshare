...
type adoptContext struct {
	...
	fileIgnorePatterns []string                       // effective file ignores for copy-mode re-sync
	projectSkills      []config.SkillEntry            // declarative entries that a bare project install will replay
	allTargets         map[string]string              // name -> skills dir, for detecting external links
...
func runAdoptCommand(actx adoptContext, opts adoptOptions, start time.Time) error {
	previousDiagOutput := ssync.DiagOutput
	if opts.jsonOutput {
		ssync.DiagOutput = io.Discard
		defer func() { ssync.DiagOutput = previousDiagOutput }()
	}
	...
	// Selection: --all and JSON are explicit non-interactive selections.
...
// selectAdoptCandidates resolves which candidates to adopt. With --all (or JSON
// output) all are selected without prompting; otherwise an interactive
...
func selectAdoptCandidates(candidates []adopt.Candidate, opts adoptOptions) ([]adopt.Candidate, bool) {
	// Explicit non-interactive opt-in. Conflicts selected by --all remain safe:
	// PullSkills skips them unless --force was separately requested.
	if opts.jsonOutput || opts.all {
		return candidates, false
	}
	// Non-interactive terminal (CI, pipe): --dry-run proceeds. A bare run must
	// NOT silently migrate + trash
...
// (copy into source, remove previewed external links, trash the original,
// re-sync to all targets), then adapts the result to the CLI's render/oplog shape.
func applyAdopt(actx adoptContext, selected []adopt.Candidate, opts adoptOptions) (*adoptResult, error) {
	...
	out, err := adopt.Apply(selected, adopt.Request{
		...
		FileIgnorePatterns: actx.fileIgnorePatterns,
		ProjectSkills:      actx.projectSkills,
