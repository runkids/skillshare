...
func printUsage() {
	...
	cmd("list", "[agents|all] [pattern]", "List installed skills (or agents)")
	cmd("search", "[query]", "Search or browse GitHub for skills")
	cmd("sync", "[agents|all] [extras]", "Sync skills/agents/extras to targets")
	...
	cmd("diff", "[agents|all]", "Show differences between source and targets")
	fmt.Println()
	...
	fmt.Println("  skillshare sync all                                 # Sync skills + agents")
	fmt.Println("  skillshare list agents                              # List agents only")
