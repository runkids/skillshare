...
const renderDialog = (props: Partial<Parameters<typeof MCPImportDialog>[0]> = {}) =>
  render(<QueryClientProvider client={new QueryClient()}><I18nProvider><ToastProvider><MCPImportDialog servers={{ github: { command: 'npx' } }} defaultTargets={['claude', 'cursor']} paths={{ claude: '/home/me/.claude.json', codex: '/home/me/.codex/config.toml' }} detected={['claude']} onClose={vi.fn()} onImported={vi.fn()} {...props} /></ToastProvider></I18nProvider></QueryClientProvider>);
...
describe('MCP import dialog', () => {
  ...
  it('asks which client pasted TOML comes from', async () => {
    ...
    const user = userEvent.setup();
    renderDialog();
    await user.click(screen.getByRole('button', { name: 'Paste configuration' }));
