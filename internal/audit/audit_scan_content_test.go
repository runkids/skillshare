...
func TestScanContent_HardcodedSecret(t *testing.T) {
	...
	tests := []struct {
		...
		{"google api key", "config: AIzaSyFakeKeyForTesting1234567890abcdef"},
		{"aws access key", "aws_key = AKIAIOSFODNN7EXAMPLE"},
		{"github classic pat", "token: ghp_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijkl"},
		{"github secret pat", "secret: ghs_ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijkl"},
		{"github fine-grained pat", "pat: github_pat_11ABCDEFGH0123456789_abcdefghij"},
		...
		{"stripe live key", "sk_live_abcdefghijklmnopqrst"},
		{"stripe test key", "rk_test_abcdefghijklmnopqrst"},
