...
func TestReadLock_FlatMapAllowsSkillNamedSkills(t *testing.T) {
	dir := filepath.Join(t.TempDir(), "skills")
	writeLock(t, dir, `{
		"skills": {"sourceTool": "skill-manager"},
		"gmail": {"sourceTool": "googleworkspace"}
	}`)

	entries, err := ReadLock(dir)
	if err != nil {
		t.Fatalf("read valid flat lockfile: %v", err)
	}
	if entries["skills"].SourceTool != "skill-manager" {
		t.Fatalf("skills source tool = %q, want skill-manager", entries["skills"].SourceTool)
	}
	if entries["gmail"].SourceTool != "googleworkspace" {
		t.Fatalf("gmail source tool = %q, want googleworkspace", entries["gmail"].SourceTool)
	}
}
