...
```bash
OUTPUT=$(ss uninstall nonexistent-skill --json 2>&1) || true
...
echo "$OUTPUT" | grep -qi "source" && echo "SOURCE=OK" || echo "SOURCE=FAIL"
echo "$OUTPUT" | grep -qi "target" && echo "TARGET=OK" || echo "TARGET=FAIL"
...
```bash
SKILLSHARE_DEV_ALLOW_WORKSPACE_PROJECT=1
